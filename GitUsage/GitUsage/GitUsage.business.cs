...
namespace GitCommits {
	...
	public partial class GitModule : Module {
		...
		public abstract partial class CommitPerDayRow : GuidedRow {
			...
			public string Data {
				...
				set {
					...
					if (value!=null) value = value.TrimEnd();
					if (string.IsNullOrEmpty(value)) throw new RequiredException(this, "Data");
					if (value.Length>DataLength) throw new ValueToLongException(this, "Data", DataLength);
					GetEdit(record==null, false);
					record.Data = value;
			...
			public const int DataLength = 80;
			...
			class DataRequiredVerifier : RequiredVerifier {
				...
				protected override bool IsValid() => !(string.IsNullOrEmpty(((CommitPerDayRow)Row).Data));
			}
		...
		public static class CommitPerDaySchema {
			...
			internal static RowDelegate<CommitPerDayRow, string> DataBeforeEdit;
			public static void AddDataBeforeEdit(RowDelegate<CommitPerDayRow, string> value)
				=> DataBeforeEdit = (RowDelegate<CommitPerDayRow, string>)Delegate.Combine(DataBeforeEdit, value);
		...
		private static Soneta.Business.App.KeyInfo keyInfoAveragePerDayWgKod = new Soneta.Business.App.KeyInfo(tableInfoAveragesPerDay, table => new AveragePerDayTable.WgKodKey(table)) {
			Name = "WgKod",
			Unique = true,
			PrimaryKey = true,
			KeyFields = new[] {"Osoba"},
		...
		public abstract partial class AveragePerDayTable : GuidedTable {
			...
			public class WgKodKey : Key<AveragePerDay> {
				internal WgKodKey(Table table) : base(table) {
				}
			...
			public WgKodKey WgKod => (WgKodKey)Session.Keys[keyInfoAveragePerDayWgKod];
		...
		public abstract partial class AveragePerDayRow : GuidedRow {
			...
			public string Osoba {
				...
				set {
					...
					record.Osoba = value;
					if (State!=RowState.Detached) {
						ResyncSet(keyInfoAveragePerDayWgKod);
					}
